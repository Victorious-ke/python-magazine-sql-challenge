...
class Article:
    def __init__(self, author: Author, magazine: Magazine, title: str, id: Optional[int] = None):
        ...
        # Ensure author and magazine have ids (or at least allow saving which will require ids)
    ...
    @classmethod
    def new_from_db(cls, row: Any) -> 'Article':
        """
        row expected columns: id, author_id, magazine_id, title
        We instantiate with Author.find_by_id and Magazine.find_by_id to attach objects.
        """
        if row is None:
            return None
        try:
            id_ = row['id']; author_id = row['author_id']; magazine_id = row['magazine_id']; title = row['title']
        except Exception:
    ...
    def save(self) -> None:
        ...
        if not self._author.id:
            # if author not saved yet, save it
            self._author.save()
    ...
    def __repr__(self):
        return f"<Article id={self.id} title={self._title!r} author_id={getattr(self._author,'id',None)} magazine_id={getattr(self._magazine,'id',None)}>"
